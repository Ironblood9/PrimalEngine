...
namespace {
	...
	public:
		...
		IDxcBlob* compile(shader_file_info info, std::filesystem::path full_path)
		{
			...
			std::wstring filename = full_path.wstring();
			LPCWSTR args[]
			{
				filename.c_str(),
				L"-E", func.c_str(),
	...
	bool compiled_shaders_are_up_to_date()
	{
		...
		if (!std::filesystem::exists(agilis_shaders_path)) return false;
		auto shaders_compilation_time = std::filesystem::last_write_time(agilis_shaders_path);
