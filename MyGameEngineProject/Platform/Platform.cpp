...
namespace primal::platform {
	...
	namespace 
	{
		...
		void resize_window(window_id id, u32 width, u32 height)
		{
			...
			// when we host the window in the level editor we just update the internal data(the client area dimensions)
	...
	window create_window(const window_init_info* const init_info /*nullptr*/)
	{
		...
		RECT rect{ info.client_area };// Copy client_area this rect
		// adjuct the window size for correct device size
...
{ 
	math::u32v4 s{ size() };
	return s.z - s.x;// right is z component and left is x component
}
