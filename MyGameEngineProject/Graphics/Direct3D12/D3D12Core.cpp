...
namespace primal::graphics::d3d12::core
{
	...
	void render_surface(surface_id id)
	{
		...
		//Presenting swap chain buffers happen in lockstep with frame buffers
		surface.present();
	...
	void create_a_root_signature()
	{
		...
		{// param 1 : 1 Contant buffer view (descriptor)
			auto& param = params[1];
		...
		{// param 2 :descriptoýr table
			...
			range.NumDescriptors = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;// we dont know have many
			range.Flags = D3D12_DESCRIPTOR_RANGE_FLAG_DESCRIPTORS_VOLATILE;
			range.OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;// we dont know have many
		...
		D3D12_GPU_VIRTUAL_ADDRESS addres_of_constant_buffer{};
		cmd_list->SetGraphicsRoot32BitConstant(0, dt_uint, 0);
		cmd_list->SetGraphicsRoot32BitConstant(0, frame_nr, 1);
		cmd_list->SetGraphicsRootConstantBufferView(1, addres_of_constant_buffer);
